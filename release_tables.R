...
# one column for each year, with one empty spacer column between groups:
#
...
sheet_col <- function(group, year_pos, n_years) {
  1 + (group - 1) * (n_years + 1) + year_pos
}
...
build_panel <- function(obs, title, rows, years,
  ...
  values <- matrix(NA_real_, nrow = nrow(rows), ncol = sheet_col(n_groups, n_years, n_years) - 1)
  ...
  for (g in seq_len(n_groups)) {
    ...
    for (i in seq_len(nrow(rows))) {
      ...
      for (j in seq_len(n_years)) {
        ...
        values[i, sheet_col(g, j, n_years) - 1] <- v
      }
    }
  }
...
write_release_table <- function(tbl, out_dir) {
  ...
  for (p in tbl$panels) {
    ...
    # Header: the stub header in column A, then an optional group row,
    # then the year row.
    first_header_row <- r
    ...
    if (n_groups > 0) {
      for (g in seq_len(n_groups)) {
        cols <- sheet_col(g, 1, p$n_years):sheet_col(g, p$n_years, p$n_years)
        writeData(wb, sh, p$group_labels[g], startRow = r, startCol = cols[1])
        mergeCells(wb, sh, rows = r, cols = cols)
        style(centre, r, cols)
        style(line, r, cols)
      }
      mergeCells(wb, sh, rows = r:(r + 1), cols = 1)  # stub header over two rows
      r <- r + 1
    }

    for (g in seq_len(max(n_groups, 1))) {
      writeData(wb, sh, as.data.frame(t(p$year_labels)), colNames = FALSE,
                startRow = r, startCol = sheet_col(g, 1, p$n_years))
    }
    style(bold, first_header_row:r, 1:last_col)
  ...
  setColWidths(wb, sh, cols = 2:last_col, widths = 9)
  n_groups <- length(tbl$panels[[1]]$group_labels)
  n_years  <- tbl$panels[[1]]$n_years
  if (n_groups > 1) {
    spacer_cols <- sheet_col(1:(n_groups - 1), n_years, n_years) + 1
